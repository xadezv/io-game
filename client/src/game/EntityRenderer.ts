...
const SPRITE_DEFS: Record<number, SpriteDef> = {
  ...
  [ET_MAMMOTH]:    { key: 'mammoth',    w: 120, h: 100, rotate: true  },
};
...
export class EntityRenderer {
  ...
  renderAnimal(e: ClientEntity, camera: Camera): void {
    ...
    if (img && def) {
      ...
      const color = e.type === ET_MAMMOTH ? '#3b3b3b' : (e.type === ET_WOLF ? '#5d4037' : '#f5f0e8');
      ...
      if (e.type === ET_MAMMOTH) {
        ctx.save();
        ctx.strokeStyle = '#f3e5c0';
        ctx.lineWidth = 3 * z;
        ctx.beginPath();
        ctx.moveTo(screen.x + 8 * z, screen.y + 8 * z);
        ctx.lineTo(screen.x + 22 * z, screen.y + 18 * z);
        ctx.moveTo(screen.x - 8 * z, screen.y + 8 * z);
        ctx.lineTo(screen.x - 22 * z, screen.y + 18 * z);
        ctx.stroke();
        ctx.restore();
    ...
    this._drawHpBar(e, screen.x, screen.y, (def ? def.h * 0.5 : (e.type === ET_MAMMOTH ? 42 : 22)) * z + 8 * z, camera);
  }
