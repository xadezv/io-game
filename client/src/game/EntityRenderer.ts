...
const SPRITE_DEFS: Record<number, SpriteDef> = {
  ...
  [ET_SPIDER]:     { key: 'spider',     w: 40, h: 40, rotate: true  },
...
export class EntityRenderer {
  ...
  renderAll(
    ...
    const sorted = Array.from(entities.values()).sort((a, b) => {
      const rankA = a.type === ET_PLAYER ? 2 : (a.type === ET_RABBIT || a.type === ET_WOLF || a.type === ET_MAMMOTH || a.type === ET_SPIDER ? 1 : 0);
      const rankB = b.type === ET_PLAYER ? 2 : (b.type === ET_RABBIT || b.type === ET_WOLF || b.type === ET_MAMMOTH || b.type === ET_SPIDER ? 1 : 0);
    ...
    for (const e of sorted) {
      ...
      switch (e.type) {
        ...
        case ET_WOLF:
        case ET_MAMMOTH:
        case ET_SPIDER:     this.renderAnimal(e, camera);  break;
  ...
  renderAnimal(e: ClientEntity, camera: Camera): void {
    ...
    if (img && def) {
      ...
      const color = e.type === ET_MAMMOTH ? '#4a4a4a' : (e.type === ET_SPIDER ? '#2f2f2f' : (e.type === ET_WOLF ? '#5d4037' : '#f5f0e8'));
      const r     = (e.type === ET_MAMMOTH ? 40 : (e.type === ET_SPIDER ? 18 : (e.type === ET_WOLF ? 22 : 14))) * z;
